...
/**
 * ProjectDiagnosisView - 学情诊断阶段空壳（Task 4 / 计划 Step 4）。
 *
 * 业务实现归 Task 6。本空壳只负责：
 * - 从统一项目上下文 store 读取本阶段真实状态、阻断项与权限（不重复请求项目详情）。
 * - 渲染正确的空态与只读态，不放入任何模拟诊断结果、随机分层或假 AI 建议。
 * - 归档时全部内容只读。
 *
 * 工作区外壳（ProjectWorkspaceLayout）已加载上下文并处理 loading/forbidden/error，
 * 故本页聚焦阶段内容；上下文缺失时降级为空态以保证健壮。
 */
...
<template>
  <section class="phase-shell" data-ui="phase-diagnosis">
    <header class="phase-shell__head">
      <h2 class="phase-shell__title">学情诊断</h2>
      <span
        v-if="phase"
    ...
    <div class="phase-shell__empty">
      <p class="phase-shell__empty-text">本阶段暂无学习证据与诊断结果。</p>
      <p class="phase-shell__empty-hint">
        完成跨学科设计后，可在此导入前测、确认学情诊断并生成分层建议。
      </p>
      <button
